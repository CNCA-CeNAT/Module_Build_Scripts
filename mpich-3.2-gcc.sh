...
soft=mpich-3.2
src=/opt/src
prefix=/opt/tools/mpich-3.2-gcc
cd $src
mkdir -p $prefix
wget --quiet http://www.mpich.org/static/downloads/3.2/mpich-3.2.tar.gz
tar -xvzf mpich-3.2.tar.gz
cd $soft
CC=gcc CXX=g++ FC=gfortran ./configure --with-pm=hydra --with-pmi=simple --enable-shared --enable-static --enable-threads=multiple --prefix=$prefix
make -j20
make install
mkdir -p /opt/tools/modulefiles/mpich

/bin/cat <<"#EOF#" > /opt/tools/modulefiles/mpich/3.2.1-gcc
#%Module#####################################################################
